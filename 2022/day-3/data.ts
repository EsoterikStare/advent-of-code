const upperAlpha = Array.from(Array(26)).map((_, i) => String.fromCharCode(i + 65));
const lowerAlpha = Array.from(Array(26)).map((_, i) => String.fromCharCode(i + 97));

export const alphaCharacters = [...lowerAlpha, ...upperAlpha] as const;

export const priorityMap = Object.fromEntries(alphaCharacters.map((char, index) => [char, index + 1]));
